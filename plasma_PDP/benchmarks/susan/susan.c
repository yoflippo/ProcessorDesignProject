...
int  tmp;
  ...
  if(!(header[0]=='P' && header[1]=='5')) {
    printf("Image %s does not have binary PGM header: %x%x.\n", filename, header[0], header[1]);
    //exit_error("Image %s does not have binary PGM header: %x%x.\n", filename, header[0], header[1]);
    exit(0);
  }
...
extern int check_identical_files(char* fn1, char* fn2, char* res);

int _main_susan(int argc ,char *argv[]);
...
#endif
{
	...
	char result[256];
	char out_fn[]="Lenna192.smoothing.pgm";
	char golden_fn[]="Lenna192golden.pgm";
	char *args1[]={"susan","Lenna192.pgm","","-s"};
	//char *args1[]={"susan","input_small.pgm","output_small.smoothing.pgm","-s"};
    //char *args2[]={"susan","input_small.pgm","output_small.edges.pgm","-e"};
    //char *args3[]={"susan","input_small.pgm","output_small.corners.pgm","-c"};
    //printf("Calling %s %s %s %s\n",args1[0],args1[1],args1[2],args1[3]);
	printf("Starting SUSAN smoothing\n");
	args1[2]=out_fn;
    t1 = get_ticks();
    _main_susan(4,args1);
    t2 = get_ticks();
    check_identical_files(golden_fn, out_fn,result);
	puts(result);
    printf("----------------------------------------------------------\n");
    printf("SUSAN took %'.6lf million clock cycles\n",(t2-t1)/1000000.0);
    printf("----------------------------------------------------------\n");
    //printf("Calling %s %s %s %s\n",args2[0],args2[1],args2[2],args2[3]);
    //t1 = get_ticks();
    //_main_susan(4,args2);
    //t2 = get_ticks();
    //printf("SUSAN edges took %'.6lf million clock cycles\n",(t2-t1)/1000000.0);
    //printf("Calling %s %s %s %s\n",args3[0],args3[1],args3[2],args3[3]);
    //t1 = get_ticks();
    //_main_susan(4,args3);
    //t2 = get_ticks();
    //printf("SUSAN corners took %'.6lf million clock cycles\n",(t2-t1)/1000000.0);
    //FlashDumpToConsole();
#ifdef ALL_BENCHMARKS
